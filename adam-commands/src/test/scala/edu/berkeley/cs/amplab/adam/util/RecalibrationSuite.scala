...
import edu.berkeley.cs.amplab.adam.util.recalibration.DiscreteCycle
...
object CompanionClass extends AdamCommandCompanion {
  ...
  val commandName = "foo"
  val commandDescription = "bar"

  def apply(c: Array[String]) : AdamCommand = null

}

class RecalibrationSuite extends FunSuite with BeforeAndAfterEach with SparkCommand {
  def run() : Unit = { }
